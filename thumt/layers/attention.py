...
def add_timing_signal(x, min_timescale=1.0, max_timescale=1.0e4, name=None):
    ...
    with tf.name_scope(name, default_name="add_timing_signal", values=[x]):#[batch,seq_len,100]
        length = tf.shape(x)[1]
        channels = tf.shape(x)[2]
        position = tf.to_float(tf.range(length)) #seq_len
        num_timescales = channels // 2   #50
        ...
        scaled_time = (tf.expand_dims(position, 1) *#[seq_len,1]*[1,50]=[seq_len,50]
