...
def maxout(inputs, output_size, maxpart=2, use_bias=True, concat=True,
    ...
    candidate = linear(inputs, output_size * maxpart, use_bias, concat,#[bs,seq,256]
                       dtype=dtype, scope=scope or "maxout")
    shape = tf.concat([tf.shape(candidate)[:-1], [output_size, maxpart]],#[bs,seq,128,2]
                      axis=0)
    value = tf.reshape(candidate, shape)
    output = tf.reduce_max(value, -1)#[bs,seq,128,1]

    return output#[bs,seq,128]
