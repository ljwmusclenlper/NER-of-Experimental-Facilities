...
sys.path.insert(0, '../..')
import thumt.data.dataset as ds
...
def parse_args():
    ...
    parser.add_argument("--checkpoints", type=str, default=["checkpoint/model.ckpt-0"],
    ...
    parser.add_argument("--parameters", type=str, default="decode_batch_size=2,use_bert=False", 
...
def import_params(model_dir, model_name, params):
    ...
    m_name = os.path.join(os.path.dirname(model_dir), model_name + ".json")
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        if params.use_bert and params.bert_emb_path:
            features = ds.get_inference_input_with_bert(params.input + [params.bert_emb_path], params)
        else:
            features = ds.get_inference_input(params.input, params)

        predictions = search.create_inference_graph(model_fns, features,  params)
