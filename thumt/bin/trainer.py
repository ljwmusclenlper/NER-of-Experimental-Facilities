...
def parse_args(args=None):
    ...
    # input files
    parser.add_argument("--input", type=str, default=['../../data/scope/train_src.txt', '../../data/scope/train_dsc.txt'],
                        help="Path of source and target corpus")
    parser.add_argument("--glove_emb_path", type=str, default='../../data/scope/glo_news_50d_ch.txt',#字向量预训练的
                        help="Path of glove embeddings")
    parser.add_argument("--bert_emb_path", type=str, default="../../data/scope/bert_vec.txt",
    ...
    parser.add_argument("--vocabulary", type=str, default="../../data/scope/vocab.w ../../data/scope/vocab.t ../../data/scope/vocab.c",
                        help="Path of source and target vocabulary")#所有中文9000+的字，中文基本5-6000字
    parser.add_argument("--validation", type=str, default='../../data/scope/test_src.txt', 
                        help="Path of validation file")
    parser.add_argument("--references", type=str, nargs="+",
                        help="Path of reference files")

    # model and configurationrnn,sec2sec,transformer
    parser.add_argument("--model", type=str, default='rnnsearch',#共三种模型rnn,sec2sec,transformer
                        help="Name of the model")
    parser.add_argument("--parameters", type=str, default="rnn_cell=DT,save_checkpoint_steps=500,train_steps=30000,batch_size=1024,max_length=512,constant_batch_size=False,embedding_size=50,learning_rate=8e-4,fine_tuning=True,stack=False,char_embedding_size=50,use_bert=False,use_glove=True,\
    hidden_size=256,dropout=0.5,rnn_dropout=0.3,global_type=self,global_hidden_size=128,global_transition_num=2,transition_num=2,bert_size=768,append_eos=True,decode_batch_size=8",
...
def default_parameters():
    params = tf.contrib.training.HParams(#定义超参数的接口
        input=["", ""],
        output="",
        record="",
        model="rnnsearch",
...
def merge_parameters(params1, params2):#paramas2优先2
...
def override_parameters(params, args):
    ...
    params.vocabulary = {
        ...
        "target": vocabulary.load_vocabulary(params.vocab[1]),#['E-PRODUCT', 'O', 'B-PRODUCT', 'S-PRODUCT', 'I-PRODUCT']
    ...
    control_symbols = [params.pad, params.bos, params.eos, params.unk]#pad,bos,eos,unk
    #记录字典中#pad,bos,eos,unk的位置
    params.mapping = {
        "source": vocabulary.get_control_mapping(#{'<unk>': 3890}source3890的位置有个unk
        ...
        "char": vocabulary.get_control_mapping(#{'<unk>': 3890}source3890的位置有个unk
...
def main(args):
    ...
    model_cls = models.get_model(args.model)#<class 'thumt.models.rnnsearch.RNNsearch'>
    params = default_parameters()#默认配置
    # Import and override parameters
    # Priorities (low -> high):
    # default -> saved -> command
    params = merge_parameters(params, model_cls.get_parameters())#合并，优先model_cls.get_parameters()
    ...
    with tf.Graph().as_default():
        if not params.record:
            ...
            features = record.get_input_features( os.path.join(params.record, "*train*"), "train", params )
        ...
        if params.update_cycle == 1:
            ...
            if isinstance(params.clip_grad_norm or None, float):
                gradients, _ = tf.clip_by_global_norm(gradients, params.clip_grad_norm)
