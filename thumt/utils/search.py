...
def beam_search(symbols_to_logits_fn, initial_ids, beam_size, decode_length,
    ...
    initial_log_probs = tf.constant([[0.] + [-float("inf")] * (beam_size - 1)])#[[0.0, -inf, -inf, -inf]]
...
def create_inference_graph(model_fns, features, params):
    ...
    decode_length = params.decode_length#是0？？
    ...
    initial_ids = tf.fill([batch_size], tf.constant(bos_id, dtype=tf.int32))# batch_size个

    inputs_old = features["source"]#[bs,字序号]
    inputs_length_old = features["source_length"] 
    inputs_chars_old = features["chars"]
    inputs_char_length_old = features["char_length"]#[bs,每个句子中的最长单词的字母数]
    if params.use_bert:
