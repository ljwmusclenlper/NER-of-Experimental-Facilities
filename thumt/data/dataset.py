...
def batch_examples(example, batch_size, max_length, mantissa_bits,
    ...
    with tf.name_scope("batch_examples"):
        ...
        # add stages [8, 10, ..., 112]
        while x < max_length:
            boundaries.append(x)#[8, 10, 12, 14, 16, 20, 24, 28, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, 448]
            x += 2 ** max(0, int(math.log(x, 2)) - mantissa_bits)

        # Whether the batch size is constant
        if not constant:
            batch_sizes = [max(1, batch_size // length)#[128, 102, 85, 73, 64, 51, 42, 36, 32, 25, 21, 18, 16, 12, 10, 9, 8, 6, 5, 4, 4, 3, 2, 2, 2]
                           for length in boundaries + [max_length]]
            # [512, 409, ..., 32]                           
            batch_sizes = [b * shard_multiplier for b in batch_sizes]
            bucket_capacities = [2 * b for b in batch_sizes]#[256, 204, 170, 146, 128, 102, 84, 72, 64, 50, 42, 36, 32, 24, 20, 18, 16, 12, 10, 8, 8, 6, 4, 4, 4]
            # [1024, ..., 64]
        else:
        ...
        (_, outputs) = tf.contrib.training.bucket_by_sequence_length(
            ...
            capacity=3,  # Number of full batches to store, we don't need many.
...
def get_training_input(filenames, params):
    ...
    with tf.device("/cpu:0"):
        def parse_fn(line_words, line_tags):
            ...
            words = [w for w in line_words.strip().split()] + ["<eos>"]#这里为什么要加<eos>,eos在target里没有，我自己加上了
            tags = [t for t in line_tags.strip().split()] + ["<eos>"]
            ...
            chars = [[c for c in w] + ["<eos>"] for w in line_words.strip().split()]
            chars.append(["<eos>"])
            char_lengths = [len(c) for c in chars]#一句话中每个单词的字母长度，是个一维向量
            chars = [c + [params.pad] * (max(char_lengths) - l) for c, l in zip(chars, char_lengths)]
        ...
        dataset = dataset.map(#映射，冒号前输入，冒号后输出
            lambda src, tgt: {
                "source": src[0],
                "target": tgt,
                "chars": src[1],
        ...
        features["chars"] = char_table.lookup(features["chars"])#字变成编号，到此为止
        ...
        features = batch_examples(features, params.batch_size,
                                  ...
                                  num_threads=params.num_threads)#增加了一维，batch_size
...
def get_training_input_with_bert(filenames, params):
    ...
    with tf.device("/cpu:0"):
        def parse_fn(line_words, line_tags,line_berts):
            # Encode in Bytes for TF
            words = [w for w in line_words.strip().split()] + ["<eos>"]
            tags = [t for t in line_tags.strip().split()] + ["<eos>"]
...
def get_evaluation_input(filenames, params):
    def parse_fn(line):
        # Encode in Bytes for TF
        words = [w for w in line.strip().split()] + ["<eos>"]
        
        # Chars
        chars = [[c for c in w] + ["<eos>"] for w in line.strip().split()]
        chars.append(["<eos>"])
        char_lengths = [len(c) for c in chars]
        chars = [c + [params.pad] * (max(char_lengths) - l) for c, l in zip(chars, char_lengths)]
        return words, chars, char_lengths    
    def generator_fn(filename):
        with Path(filename).open('r',encoding="utf8") as src_f:
            for line in src_f:
                if line=="-DOCSTART-\n":
                    continue
                yield parse_fn(line)    
    ...
    dataset = dataset.padded_batch(#进行一个batch的padding
    ...
    src_table = tf.contrib.lookup.index_table_from_tensor(#建立字表
        tf.constant(params.vocabulary["source"]),
    ...
    features["source"] = src_table.lookup(features["source"])#字转化为数字
    features["chars"] = char_table.lookup(features["chars"])
